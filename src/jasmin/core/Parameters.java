...
public class Parameters
{
	...
	{
		return ((size(0) & size(1) & size(2)) != 0) ? null : new ParseError(wholeLine, argument(1), "Size mismatch");
	}
	...
	public ParseError firstRegMemSecondReg()
	{
		...
		{
			return new ParseError(wholeLine, argument(0), "First argument must be a register or memory address");
		}
	...
	public ParseError numericDestOK()
	{
		if (!Op.matches(type(0), Op.REG | Op.MEM))
		{
			return new ParseError(wholeLine, argument(0), "Invalid parameter. Must specify a register or a memory address as destination.");
		}
		return null;
	}
	...
	public long pop(int size)
	{
		...
		long result = dsp.getUpdate(stack, false);
	...
	public void set(String wholeCodeLine, String command, ArrayList<FullArgument> args, int defaultOperationSize,
		...
		for (int i = 0; i < numArguments; i++)
		{
			...
			if (!Op.matches(type(i), Op.CHARS | Op.STRING))
			{
				if (size < minSize)
				{
					...
					{
						return new ParseError(wholeLine, argument[i],
